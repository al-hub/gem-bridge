...
## 2. 모바일 단일 콘솔 (`GeminiBridge/CONSOLE`) 추천 워크플로우

가장 빠르고 편리한 스마트폰 조작 방법은 Google Drive의 **`GeminiBridge/CONSOLE` 단일 구글 문서**를 활용하는 것입니다.

### (1) 특징 및 장점
- **원탭 제어**: 스마트폰 홈 화면에 `CONSOLE` 문서 바로가기를 두고 터미널처럼 사용.
- **실시간 반응**: 상단 `[🟢 ONLINE]` 상태 확인 및 입력 후 3초 내 자동 실행.
- **오프라인 감지**: PC가 꺼져 있을 때는 `[🔴 OFFLINE]`으로 표시되며, 작성해 둔 명령은 PC 부팅 시 자동 실행(Auto-Queueing).
- **무한 루프 방지**: 3중 Anti-Echo 가드레일로 안전한 양방향 동기화.

### (2) 스마트폰 입력 방법
문서 최상단 입력 영역에 원하는 명령어를 적기만 하면 자동 저장 후 실행됩니다:
```text
>>> INPUT >>>
!작업 kum index.html 랜딩페이지 문구 변경
<<< END <<<
```
